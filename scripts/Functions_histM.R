...
Combine.regeneration.peaks.all.four.markers = function()   
{
  ...
  for(n_histM in 1:length(conds_histM))
  {
    ...
    names[ii_missed] = paste0(conds_histM[n_histM], '_', names[ii_missed])
    colnames(xx) = names
    xx$dynamic = NA
    xx$dynamic[select] = 1
    colnames(xx)[ncol(xx)] = paste0('dynamic_', conds_histM[n_histM])
    # pp = data.frame(t(sapply(rownames(res), function(x) unlist(strsplit(gsub('_', ':', as.character(x)), ':')))))
    # rownames(res) = paste0(pp[,1], ':', pp[,2], '-', pp[,3])
  ...
  saveRDS(keep, file = paste0(RdataDir, '/regeneration_combined_4histMarkers_DE_345k.Rdata'))
